...
writeConfig <- function(config, configfile) {
    ...
    # }
    cat("\n[SaaRclust]\n", file=f)
    for (i1 in c('min.contig.size','pairedEndReads','bin.size','step.size','store.data.obj','reuse.data.obj','num.clusters','alpha','best.prob','prob.th','ord.method','assembly.fasta','concat.fasta','z.limit','remove.always.WC')) {
        cat(i1," = ",formatstring(config[[i1]]),"\n", file=f)
    }
    close(f, type='w')
}
