...
#'
#' @param clust.prob A \code{list} of binomial probabilities of each PB read in every possible cluster
#' @param pi Paramter estimate of the size of each cluster
#' @return A \code{list} of ...
#' @author David Porubsky, Maryam Ghareghani
#' @export

gammaFunction <- function(clust.prob=NULL, pi=NULL) {
  ...
  for (i in 1:length(clust.prob)) {
    ...
    clust.gamma <- (clust.p*pi[i])/clust.sums
